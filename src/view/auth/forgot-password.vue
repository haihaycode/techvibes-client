    ...
    <div class="flex h-screen">
        ...
        <div class="w-full bg-gray-100 lg:w-1/2 flex items-center justify-center">
            <div class="max-w-md w-full p-6">
                <svg class="w-28 h-28 mx-auto text-gray-800 dark:text-white" aria-hidden="true"
                ...
                <h1 class="text-3xl font-semibold mb-6 text-black text-center">Quên mật khẩu</h1>
                <h1 class="text-sm font-semibold mb-6 text-gray-500 text-center"> </h1>
                <Form @submit="handleForgotPass" :validation-schema="schema" v-slot="{ errors }" class="space-y-4">
                    <div class="flex flex-col lg:flex-row lg:items-end">
                        <div class="flex-1 lg:flex-[3_1]">
                            <label for="email" class="block text-sm font-medium text-gray-700">Tài khoản (email)
                                <span class="text-red-500">{{ errors.email }}</span> </label>
                            <Field type="text" id="email" name="email" ref="email"
                                class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                                :class="{ 'bg-red-50 border border-red-500 text-red-900 placeholder-red-700': errors.email, ' ': !errors.email }" />

                        </div>
                        <div class="mt-4 lg:mt-0 lg:ml-4 lg:flex-[1_1]">
                            <Button type="button"
                                :text="otpSent ? `${timer}s` : (loadingSendOtp ? 'Đang xử lý...' : 'Gửi mã')"
                                :loading="loadingSendOtp"
                                :disabled="loadingSendOtp || otpSent || errors.email || !$refs.email"
                                @click="handleSendOtp"
                                class="w-full bg-black text-white p-2 rounded-md hover:bg-gray-800 focus:outline-none focus:bg-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 transition-colors duration-300"></Button>
                        </div>
                    </div>
                    <div>
                        <label for="otp" class="block text-sm font-medium text-gray-700">Mã đã nhận</label>
                        <Field name="otp" type="number"
                            class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                            :class="{ 'bg-red-50 border border-red-500 text-red-900 placeholder-red-700': errors.otp, ' ': !errors.otp }" />
                        <div class="text-red-500">{{ errors.otp }}</div>
                    </div>
                    <div>
                        <label for="password" class="block text-sm font-medium text-gray-700"> Mật khẩu
                            mới</label>
                        <Field name="password" type="password"
                            class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                            :class="{ 'bg-red-50 border border-red-500 text-red-900 placeholder-red-700': errors.password, ' ': !errors.password }" />
                        <div class="text-red-500">{{ errors.password }}</div>
                    </div>
                    <div>
                        <label for="confirmPassword" class="block text-sm font-medium text-gray-700">Xác nhận mật khẩu
                            mới</label>
                        <Field name="confirmPassword" type="password"
                            class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                            :class="{ 'bg-red-50 border border-red-500 text-red-900 placeholder-red-700': errors.confirmPassword, ' ': !errors.confirmPassword }" />
                        <div class="text-red-500">{{ errors.confirmPassword }}</div>
                    </div>
                    <div>
                        <Button type="submit" :text="loading ? '  Đang xử lý...' : 'Khôi phục mật khẩu'"
...
export default {
    ...
    data() {
        const schema = Yup.object().shape({
            ...
            otp: Yup.string().required("Mã OTP là bắt buộc").min(6, 'Mã OTP phải có ít nhất 6 ký tự *').max(6, 'Mã OTP phải có tối đa 6 ký tự *'),
            password: Yup.string().min(1, 'Mật khẩu phải có ít nhất 1 ký tự *').required('Mật khẩu là bắt buộc *'),
    ...
    methods: {
        async handleSendOtp() {
            ...
            try {
                ...
                const resp = await authService.sendMailForgotPassword(getOtp);
        ...
        async handleForgotPass(values) {
            ...
            try {
                ...
                notyf.success(resp.message);
                this.$router.push('/login?reset_password=true');
            } catch (error) {
                notyf.error(error.response.data.message);
            } finally {
                this.loading = false;
