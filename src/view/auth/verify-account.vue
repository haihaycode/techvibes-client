    ...
    <div class="flex h-screen">
        ...
        <div class="w-full bg-gray-100 lg:w-1/2 flex items-center justify-center">
            <div class="max-w-md w-full p-6">
                <svg class="text-blue-800 dark:text-white mx-auto" aria-hidden="true" width="50% " height="50%"
                ...
                <h1 class="text-3xl font-semibold mb-6 text-black text-center">Xác minh tài khoản của bạn</h1>
                <h1 class="text-sm font-semibold mb-6 text-gray-500 text-center">
                    Để có thể mở tài khoản của bạn, bạn cần nhập mã OTP mà chúng tôi gửi đến hòm thư của bạn.
                </h1>
                <Form @submit="handleVerifyAccount" :validation-schema="schema" v-slot="{ errors }" class="space-y-4">
                    <div>
                        <div class="mt-4 lg:mt-0 ">
                            <Button type="button"
                                :text="otpSent ? `${timer}s` : (loadingSendOtp ? 'Đang xử lý...' : 'Gửi mã')"
                                :loading="loadingSendOtp" :disabled="loadingSendOtp || otpSent" @click="handleSendOtp"
                    ...
                    <div>
                        <Button type="submit" :text="loading ? '  Đang xử lý...' : 'Xác minh tài khoản'"
...
export default {
    ...
    methods: {
        ...
        async handleSendOtp() {
            ...
            try {
                const resp = await authService.sentMailVerifyAccount(this.$store);
        ...
        async handleVerifyAccount(values) {
            var notyf = new Notyf();
            this.loading = true;
            try {
                const resp = await authService.verifyAccount(this.$store, values.otp);
                notyf.success(resp.message);
                await this.logoutUser();
                this.$router.push('/login?verify_account=true');
            } catch (error) {
                if (error.response && error.response.status === 403) {
                    notyf.error('Bạn chưa đăng nhập. Vui lòng đăng nhập lại.');
                    await this.logoutUser();
