<template>
    ...
    <section class="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
            <h2 class="text-xl font-bold mb-6 border-b border-gray-200 pb-4">Tiến hành đặt hàng <br>
                <i class="text-gray-500 text-sm mt-1 "> Chú ý : Kiểm tra sản phẩm > kiểm tra số lượng > kiểm tra địa chỉ
                    giao
                    hàng > Đặt hàng > Kiểm tra thông tin đơn hàng</i>
            </h2>
        ...
        item.product.name }}</a>
                    ...
                    <a v-if="cartItems.length > 0" @click="clearCart"
                        ...
                        <i class="fad fa-trash-alt"></i> &nbsp; Xóa các sản phẩm
                    </a>


                    <AddressComponent style="padding: 0px !important;"></AddressComponent>


                </div>

                <div v-if="cartItems.length" class="mx-auto mt-6 max-w-4xl flex-1 space-y-6 lg:mt-0 lg:w-full">
                    <div
        ...
        formatCurrency(totalPriceStock - totalPrice) }} </dd>
                                ...
                                <dl class="flex items-center justify-between gap-4">
                                    <dt class="text-base font-normal text-gray-500 dark:text-gray-400">Các sản phẩm :
                                    </dt>
                                    <dd class="text-base font-extrabold text-gray-500 dark:text-white ">{{
        cartItems.length }} Sản phẩm</dd>
                                </dl>
        ...
        formatCurrency(totalPrice) }} VNĐ</dd>
                                ...
                                <div class="space-y-2">
                                    <label class="flex items-center">
                                        ...
                                        <span class="ml-2 text-gray-700 dark:text-gray-300">Thanh toán khi nhận
                                            hàng</span>
                                    </label>
                                    ...
                                    <label class="flex items-center">
                                        <input type="radio" name="paymentMethod" value="vnpay" v-model="paymentMethod"
                                            class="form-radio text-red-500 h-4 w-4" />
                                        <span class="ml-2 text-gray-700 dark:text-gray-300">Thanh toán bằng VNPay</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <Button :loading="buttonLoading" :text="buttonLoading ? 'Đang tiến hành' : 'Đặt hàng'"
                                v-else type="button" @click="checkout"
...
import AddressComponent from '@/view/address.vue';
    ...
    data() {
        return {
            breadcrumbs: [
                ...
                { text: 'Giỏ hàng', link: '/account/cart' },
                { text: 'Đặt hàng', link: '/account/checkout' },
            ],
            cartItems: [],
            totalPrice: 0,
            totalPriceStock: 0,
        ...
        async checkout() {
            ...
            try {
                ...
                if (addressList.length > 0) {
                    this.address = addressList.some(element => element.defaultAddress);
                } else {
                    this.address = false;
                }
            } catch (error) {
            ...
            if (!this.address) {
                notyf.error("Bạn chưa cập nhật địa chỉ");
                return;
            }

            if (this.cartItems.length === 0) {
                notyf.error("Vui lòng thêm sản phẩm vào giỏ hàng trước khi tiến hành mua hàng");
                return;
            }
